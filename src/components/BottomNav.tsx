      ...
      <div className="flex items-center justify-around py-1.5 pb-[max(0.375rem,env(safe-area-inset-bottom))]">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              {isActive && (
                <span className="absolute -top-1.5 h-1 w-6 rounded-full bg-primary-foreground" />
              )}
              <Icon
                className={`h-5 w-5 transition-all ${
                  isActive ? "text-primary-foreground scale-110" : "text-primary-foreground/60 group-hover:text-primary-foreground/90"
                }`}
